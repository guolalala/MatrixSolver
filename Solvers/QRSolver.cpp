...
int main()
{
    ...
    ofstream fout("../logs/QR_2000.log", ios::out | ios::trunc); //在文件不存在时创建新文件，并在文件已存在时清除原有数据并写入新数据
    if(!fout)
    {
    ...
    fout<<"QRSolver for ACTIVSg2000 Succeed!"<<endl;
    ...
    cout<<"QRSolver for ACTIVSg2000 Succeed!"<<endl;
