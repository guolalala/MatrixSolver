...
int main()
{
    ...
    ofstream fout("../logs/LU_10klog", ios::out | ios::trunc); //在文件不存在时创建新文件，并在文件已存在时清除原有数据并写入新数据
    if(!fout)
    {
    ...
    fout<<"LUSolver for ACTIVSg10k Succeed!"<<endl;
    ...
    cout<<"LUSolver for ACTIVSg10k Succeed!"<<endl;
